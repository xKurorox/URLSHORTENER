...
class URL(Base):
    ...
    created_at = Column(DateTime, default = lambda: datetime.now(timezone.utc))         # Timestamp set on creation
...
class Click(Base):
    ...
    id = Column(Integer, primary_key = True, index = True)      # Auto-incrementing primary key
    click_date = Column(DateTime, default=lambda: datetime.now(timezone.utc))      # Timestamp of the click
